...
def __get_default_frames_dir(video_path: str) -> str:
    # 视频所在目录文件夹名称
    return f'{os.path.dirname(video_path)}/frames/'


def video_to_frames_with_ffmpeg(video_path: str, fps: int):
    """
    使用ffmpeg进行视频分帧
    Args:
        video_path: 视频地址
...
def video_to_frames_with_opencv(video_path, reset_height: int = 0):
    """
    将视频转换成一帧帧的图片
    video_path: 视频文件完整路径
    ...
    logger.info(
        f'fps={fps} , width={width} , height={height} , frames_count={frames_count} ，total_time={frames_count / fps}')
    ...
    while True:
        ...
        frame_index = round(frame_pos_msec)
